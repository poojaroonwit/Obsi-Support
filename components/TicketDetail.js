...
import AgentKnowledgePicker from './knowledge/AgentKnowledgePicker';
...
export default function TicketDetail({ ticket, macros = [], routing = { teams: [] }, assignment = {}, onRoutingRefresh, onRefresh, onClose }) {
  ...
  const insertKnowledge=(text)=>setReply((current)=>current.trim()?`${current.replace(/\s+$/,'')}\n\n${text}`:text);
