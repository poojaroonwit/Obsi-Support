...
export default function TicketDetail({ ticket, macros = [], routing = { teams: [] }, assignment = {}, onRoutingRefresh, onRefresh, onClose }) {
  ...
  const insertKnowledgeLink = (article) => {
    const link = `${article.title}: ${article.helpUrl}`;
  ...
  return <section className="ticket-detail">
    ...
    <div className="detail-body"><div className="conversation"><div className="conversation-heading"><strong>Conversation</strong><span>{ticket.messages?.length || 0} messages</span></div>
      ...
      <div className="composer">
        <div className="composer-tabs"><button className={!internal ? 'active' : ''} onClick={() => setMode(false)}>Reply</button><button className={internal ? 'active' : ''} onClick={() => setMode(true)}>Internal note</button></div>
        {!internal&&macros.length?<div className="macro-composer-bar"><strong>Macro</strong><select value={selectedMacroId} disabled={macroLoading} onChange={(e)=>prepareMacro(e.target.value)}><option value="">Choose a canned reply…</option>{macros.map((macro)=><option key={macro.id} value={macro.id}>{macro.shortcut?`${macro.shortcut} · `:''}{macro.name}</option>)}</select>{macroLoading?<span className="hint">Preparing…</span>:null}{macroError?<span className="macro-error">{macroError}</span>:null}</div>:null}
        {stagedActions?<div className="macro-staged">{labels.map((label)=><span key={label}>{label}</span>)}<button onClick={()=>{setStagedActions(null);setMacroError('');}}>Clear actions</button><button className="apply-actions" onClick={applyStagedActions}>Apply actions</button></div>:null}
        {!internal ? <KnowledgeSuggestions ticket={ticket} onInsert={insertKnowledgeLink} /> : null}
        <textarea value={reply} onChange={(e) => setReply(e.target.value)} placeholder={internal ? 'Add a note only your team can see…' : `Reply to ${ticket.requesterName}…`} rows={5}/>
