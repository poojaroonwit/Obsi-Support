...
const deleteSlaPolicy = async (organizationId) => {
  try {
    await query('DELETE FROM support_sla_policies WHERE organization_id=$1', [organizationId]);
    return true;
  } catch (error) {
...
module.exports = { defaultBusinessHoursPolicy, defaultSchedule, deleteSlaPolicy, getSlaPolicy, saveSlaPolicy };
