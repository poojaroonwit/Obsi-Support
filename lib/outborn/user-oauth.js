...
const buildAuthorizeUrl = ({ issuer, clientId = ACCOUNT_OAUTH_CLIENT_ID, redirectUri, state, nonce, codeChallenge }) => {
  ...
  url.searchParams.set('scope', 'openid email profile organizations'); url.searchParams.set('state', state); url.searchParams.set('nonce', nonce);
...
module.exports = { ACCOUNT_OAUTH_CLIENT_ID, ACCOUNT_OAUTH_NONCE_COOKIE, ACCOUNT_OAUTH_RETURN_COOKIE, ACCOUNT_OAUTH_STATE_COOKIE, ACCOUNT_OAUTH_VERIFIER_COOKIE, buildAuthorizeUrl, createPkcePair, exchangeAuthorizationCode, fetchAccountUserInfo, getAccountOAuthConfig, normalizeReturnPath, oauthCookieOptions, randomUrlSafe, resolveAppBaseUrl };
