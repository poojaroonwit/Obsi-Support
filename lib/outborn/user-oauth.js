...
const SUPPORT_ACCOUNT_SCOPES = 'openid email profile organizations knowledge.read knowledge.search knowledge.public.read';
...
const buildAuthorizeUrl = ({ issuer, clientId = ACCOUNT_OAUTH_CLIENT_ID, redirectUri, state, nonce, codeChallenge }) => {
  ...
  url.searchParams.set('scope', SUPPORT_ACCOUNT_SCOPES); url.searchParams.set('state', state); url.searchParams.set('nonce', nonce);
...
const getAccountOAuthConfig = (req, env = process.env) => {
  ...
  return { accountBaseUrl: requireBaseUrl(accountBaseUrl, 'OUTBORN_ACCOUNT_AUTH_URL'), appBaseUrl, clientId: String(env.OUTBORN_ACCOUNT_OAUTH_CLIENT_ID || env.OUTBORN_OBSI_SUPPORT_WEB_CLIENT_ID || ACCOUNT_OAUTH_CLIENT_ID).trim(), redirectUri: `${appBaseUrl}/api/auth/account/callback` };
};
...
module.exports = { ACCOUNT_OAUTH_CLIENT_ID, ACCOUNT_OAUTH_NONCE_COOKIE, ACCOUNT_OAUTH_RETURN_COOKIE, ACCOUNT_OAUTH_STATE_COOKIE, ACCOUNT_OAUTH_VERIFIER_COOKIE, SUPPORT_ACCOUNT_SCOPES, buildAuthorizeUrl, createPkcePair, exchangeAuthorizationCode, fetchAccountUserInfo, getAccountOAuthConfig, normalizeReturnPath, oauthCookieOptions, randomUrlSafe, resolveAppBaseUrl };
