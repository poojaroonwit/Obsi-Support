...
const calculateSlaTargets = (createdAt = new Date(), priority = 'normal', policy = DEFAULT_SLA_POLICY) => {
  ...
  const rules = policy?.targets || policy;
  const rule = rules?.[normalized] || rules?.normal || DEFAULT_SLA_POLICY.normal;
  if (policy?.enabled && policy?.schedule && policy?.timezone) {
    const { addBusinessMinutes } = require('./business-hours');
    return { firstResponseDueAt: addBusinessMinutes(createdAt, rule.firstResponseMinutes, policy), resolutionDueAt: addBusinessMinutes(createdAt, rule.resolutionMinutes, policy) };
  }
