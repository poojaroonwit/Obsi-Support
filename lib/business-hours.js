...
const normalizeTargets = (targets = {}) => Object.fromEntries(Object.entries(DEFAULT_TARGETS).map(([priority, fallback]) => {
  ...
  const firstResponseMinutes = Math.max(1, Math.floor(Number(source.firstResponseMinutes) || fallback.firstResponseMinutes));
  const resolutionMinutes = Math.max(firstResponseMinutes, Math.floor(Number(source.resolutionMinutes) || fallback.resolutionMinutes));
