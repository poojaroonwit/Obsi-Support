...
const publicKnowledgeDocument = async ({ organizationSlug, documentSlug, env, fetchImpl, timeoutMs } = {}) => {
  ...
  return result.available ? { available: true, document: result.value?.document || null } : { available: false, document: null, errorCode: result.errorCode };
};

const publicKnowledgeCollections = async ({ organizationSlug, env, fetchImpl, timeoutMs } = {}) => {
  const slug = String(organizationSlug || '').trim();
  if (!slug) return { available: true, collections: [] };
  const result = await requestJson(`/api/v1/public/${encodeURIComponent(slug)}/collections`, { env, fetchImpl, timeoutMs });
  return result.available ? { available: true, collections: Array.isArray(result.value?.collections) ? result.value.collections : [] } : { available: false, collections: [], errorCode: result.errorCode };
};
