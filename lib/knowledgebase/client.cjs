...
async function publicKnowledgeFeedback(organizationSlug, feedback, options = {}) {
  const result = await safeKnowledgeCall(() => requestKnowledge(`/api/v1/public/${encodeURIComponent(organizationSlug)}/feedback`, { ...options, method: 'POST', body: feedback }));
  return { available: result.available, feedback: result.data?.feedback || null, errorCode: result.errorCode };
}
