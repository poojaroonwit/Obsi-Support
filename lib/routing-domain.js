...
const chooseLeastLoadMember = (members = []) => {
  const eligible = members
    .filter((member) => member && member.active !== false && normalizeCapacity(member.capacity) > 0)
    .map((member) => ({ ...member, capacity: normalizeCapacity(member.capacity), load: Math.max(0, Number(member.load) || 0) }));
  eligible.sort((a,b) => {
