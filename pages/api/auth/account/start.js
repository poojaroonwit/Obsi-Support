...
export default async function handler(req, res) {
  ...
  try {
    ...
    const target = buildAuthorizeUrl({ issuer: config.accountBaseUrl, clientId: config.clientId, redirectUri: config.redirectUri, state, nonce, codeChallenge: challenge, scopes: config.scopes }); const options = oauthCookieOptions();
